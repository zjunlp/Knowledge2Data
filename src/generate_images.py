...
parser.add_argument("--input_file", default="./data/skg_boxes.json", type=str)
parser.add_argument("--output_file", default="./data/skg_images.json", type=str)
...
parser.add_argument("--regenerate", default=1, type=int,
                    help="Number of regenerations. Different from repeats, regeneration happens after everything is generated")
parser.add_argument("--force_run_ind", default=None, type=int,
...
parser.add_argument("--ignore-bg-prompt", action="store_true",
                    help="Ignore the background prompt (set background prompt to an empty str)")
parser.add_argument("--ignore-negative-prompt", action="store_true",
                    help="Ignore the additional negative prompt generated by LLM")
parser.add_argument("--no-synthetic-prompt", action="store_true",
...
parser.add_argument("--no-center-or-align", action="store_true",
...
parser.add_argument("--sdxl-step-ratio", type=float, default=0.3,
                    help="SDXL step ratio: the higher the stronger the refinement.")
...
if args.use_sdv2:
    ...
    models.sd_key = os.path.join(os.environ.get("SKG_HF_MODELS"), "stable-diffusion-2-1-base")
    models.sd_version = "sdv2"
else:
    if args.run_model in gligen_models:
        models.sd_key = os.path.join(os.environ.get("SKG_HF_MODELS"), "diffusers-generation-text-box")
        models.sd_version = "sdv1.4"
    else:
        models.sd_key = os.path.join(os.environ.get("SKG_HF_MODELS"), "stable-diffusion-v1-5")
...
base_save_dir = f"img_generations/img_generations_template{args.template_version}_{version}_{save_suffix}"
...
resps = [example["resp"] for example in data]

for i in range(len(data)):
    data[i]["img"] = f"{save_dir}/{i}"

for regenerate_ind in range(args.regenerate):

    for resp_ind, resp in enumerate(tqdm(resps, desc=f"Run: {save_dir}")):
        # for resp_ind, resp in enumerate(resps):
        # For `save_as_display`:
        save_ind = 0
        ...
        if not (
            ...
            vis.reset_save_ind()
            try:
                gen_boxes, bg_prompt, prompt = parse_input_without_negative(resp, no_input=True)
                neg_prompt = ""
                ...
                for repeat_ind in range(repeats):
                    ...
                    if args.sdxl:
                        output = sdxl.refine(image=output, spec=spec,
                                             refine_seed=original_ind_base + ind_offset + LARGE_CONSTANT4,
                                             refinement_step_ratio=args.sdxl_step_ratio)

                    path = vis.display(output, "img", repeat_ind, save_ind_in_filename=False)
